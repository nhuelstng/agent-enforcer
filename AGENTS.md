...
| Go | `AST_GO` | `.go` | `FunctionComplexityMatcher`, `NamingConventionMatcher`, `ImportMatcher`, `AstNodeMatcher`, `DocstringMatcher`, `ArchitectureMatcher` |
| C# / .NET | `AST_CSHARP` | `.cs` | `FunctionComplexityMatcher`, `NamingConventionMatcher`, `ImportMatcher`, `AstNodeMatcher`, `DocstringMatcher`, `MagicNumberMatcher`, `InterfaceMatcher`, `InvocationMatcher`, `AsyncMethodMatcher`, `EndpointAuthMatcher`, `ArchitectureMatcher`, `DeepImportBarrierMatcher`, `CycleMatcher` |

For C#, set `needs=Needs.AST_CSHARP`. `MagicNumberMatcher` flags integer/`real_literal` values only inside member bodies (method/constructor/accessor/local-function) — `const`/`readonly` declarations, parameter defaults, attribute arguments, enum members, and class-field initializers are exempt. `InterfaceMatcher` counts `public`-modifier methods in the `declaration_list` and treats any `base_list` (a base class *or* an implemented interface) as satisfying substitutability; records are a distinct node type and are ignored. `HasAttributePredicate` filters a match by the `attribute_list` on its declaration (or the declaration's parent), never climbing to an enclosing type, so a class attribute is not mis-attributed to an inner member. `HasBaseTypePredicate` reads the `base_list` (C#), `class_heritage` (TS), or class `argument_list` (Python) and matches the base class/interface text. `AttributeArgumentPredicate` inspects a named attribute's `attribute_argument_list` structurally (e.g. require a `[Route(...)]` template). `InvocationMatcher` matches the callee text of a call node (`invocation_expression`/`call`/`call_expression`) to ban calls such as `.Wait()`/`.ToList()`; it sees invocations only, not bare property access like `.Result`. `AsyncMethodMatcher` flags `async void` methods (`check="no_async_void"`) or unqualified `Task`/`ValueTask`-returning methods not named `*Async` (`check="task_suffix"`). `EndpointAuthMatcher` flags minimal-API endpoint registrations (`app.MapGet`/`MapPost`/… — `endpoint_methods`) whose fluent chain contains no `RequireAuthorization`/`AllowAnonymous` call (`guard_methods`): it climbs the `member_access_expression`/`invocation_expression` chain from each `Map*` call, so `app.MapGet(...).RequireAuthorization()` is clean but a bare `app.MapGet(...)` is flagged. Group-level (`MapGroup`) and stored-endpoint guards live outside the chain and are not tracked — prefer an inline guard at the call site. `ArchitectureMatcher`/`DeepImportBarrierMatcher`/`CycleMatcher` work on C# through the namespace import graph (`enforcer/csharp_imports.py`): a `using X.Y;` resolves to the files declaring namespace `X.Y`. `ProjectReferenceMatcher` (RAW, on `.csproj`/`.fsproj`) is the project-graph counterpart: it maps project files to layers by glob and flags a `<ProjectReference>` whose `source layer -> target layer` edge is forbidden — the assembly-boundary contract the namespace graph does not see. Each `<ProjectReference Include="..\Infra\Infra.csproj"/>` resolves relative to the source project's own directory, so no cross-file read is needed.

For Go, set `needs=Needs.AST_GO` on the supported matchers. `NamingConventionMatcher` targets Go spec nodes (`function_declaration`, `method_declaration`, `type_spec`, `const_spec`, `var_spec`, `field_declaration`) and reads method/field names from `field_identifier`. `FunctionComplexityMatcher` selects the params list after the function name, so it excludes the method receiver and the result tuple and counts grouped params (`a, b int` → 2); Go `switch`/`select` cases count toward cyclomatic and their bodies toward nesting. `DocstringMatcher` treats an exported declaration as documented when an adjacent `//` or `/* */` comment sits on the line directly above it (Go's convention), and only checks exported (upper-case) names. `ArchitectureMatcher` works on Go via the import graph: `ImportGraphBuilder` reads the module path from `go.mod` and resolves each `import "mod/pkg"` to the non-test `.go` files in that package directory (a Go package is a directory).
