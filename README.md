...
`ArchitectureMatcher` reads the import graph and flags forbidden imports. Declare a layer DAG (`layers` + `allowed_edges`, with `forbid_implicit=True`), and/or `isolate_siblings` — parent dirs whose immediate children are peer slices that may not import each other (the vertical-slice "no cross-slice imports" invariant a flat layer can't express, since sibling slices collapse to one layer). Both share one import-graph pass (Python; when a package is rooted in a subdirectory, map it with `SOURCE_ROOTS = {"app": "backend/app"}` in config so `app.*` imports resolve at the repo-root workspace). `DeepImportBarrierMatcher(module_glob="pkg/*", entry_points=["__init__.py"])` complements it on the same graph — it governs *where* a permitted crossing may land: imports into a module must hit an `entry_point`, not reach into internals (symbol-agnostic, no allowlist).
