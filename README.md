...
Set `needs=Needs.AST_CSHARP` on any AST matcher to check `.cs` files (`DocstringMatcher`, `NamingConventionMatcher`, `FunctionComplexityMatcher`, `ImportMatcher`, `AstNodeMatcher`, `MagicNumberMatcher`, `InterfaceMatcher`, and the namespace-graph matchers `ArchitectureMatcher`/`DeepImportBarrierMatcher`/`CycleMatcher`). `HasAttributePredicate(pattern=...)` filters a match by the C# attribute on its declaration — the enabler for attribute-driven ASP.NET rules (e.g. flag `*Controller` classes lacking `[Authorize]` with `AstNodeMatcher(node_type="class_declaration")` + `NodeNamePredicate(pattern=r"Controller$")` + `NotP(HasAttributePredicate(pattern=r"Authorize|AllowAnonymous"))`). `CsprojProps` extracts `<PropertyGroup>` names and `PackageReference` ids so `KeySetSyncMatcher` can require project properties (`Nullable`, `TreatWarningsAsErrors`) or keep `Directory.Packages.props` in sync. See [`AGENTS.md`](AGENTS.md#language-support) for C# node-type specifics.
